describe("GIVEN a user is viewing the StorageRecycleJobPage", function(){
	beforeEach(function() {
		document.body.innerHTML += '<div id="maxRecordsError" style="display:none;"><font style="color:red">The max value for this field is 50,000.</font></div><br/>' + 
		                            ...
		                            '<div id="newSettingNameError" style="display:none;"><font style="color:red">A Data Cleanup Rule already exists with this name.</font></div>' + 
		                            '<div id="NewSettingObjectError" style="display:none;"><font style="color:red">This is a required Field.</font></div>' + 
		                            '<div id="editSettingNameError" style="display:none;"><font style="color:red">A Data Cleanup Rule already exists with this name.</font></div>' + 
		                            ...
		                            '<div id="childConfig" style="display:none;"></div>' + 
		                            ...
		                            '<div id="helpScreen" style="display:none"></div>' +
																'<div id="agModal" style="display:none"></div>' +
		                            '<div id="errorTag"></div>' +
	...
	describe("WHEN the user enters search term that equals a record name that was archived", function(){
		
		it("THEN the record that meet the serch terms and ID should be returned.", function(){
			var backupBody = 'id,subject\n';

			backupBody += '0,Test record\n';
			backupBody += '1,Test Record\n';
			backupBody += '2,Test Record\n';
			backupBody = btoa(backupBody);

			var archiveRecordList = [];
			archiveRecordList = buildRecordFoundInBackup(backupBody, 'Test Record', 'subject', '2');
			
			expect(archiveRecordList[0]).toBe('id&&&2');
			expect(archiveRecordList[1]).toBe('subject&&&Test Record');
		});
